...
class QueryProcessor:
    ...
    def process_query(self, query):
        if query.type == "check":
            # use reverse order, because we append strings to the end
            self.write_chain(
                reversed(self.elems[query.ind])
            )
        else:
            # ind = self.elems.index(query.s)
            i_bucket = self._hash_func(query.s)
            try:
                ind = self.elems[i_bucket].index(query.s)
            except ValueError:
                ind = -1
            ...
            if query.type == 'find':
                ...
                if ind == -1:
                    self.elems[i_bucket].append(query.s)
            else:
                if ind != -1:
                    self.elems[i_bucket].pop(ind)
